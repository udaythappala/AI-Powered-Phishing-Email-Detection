package com.soc.phishing.controller;
...
import com.soc.phishing.service.EmailAnalysisService;
...
public class EmailAnalysisController {
    ...
    public ResponseEntity<EmailAnalysisResponse> analyzeEmail(
        ...
        return ResponseEntity.ok(service.analyzeEmail(request));
    }

    @GetMapping("/history")
    public ResponseEntity<List<EmailAnalysis>> getHistory() {

        return ResponseEntity.ok(service.getAllAnalyses());
    }

    @GetMapping("/{id}")
    public ResponseEntity<EmailAnalysis> getAnalysis(@PathVariable Long id) {
